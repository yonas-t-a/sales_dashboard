...
export function VisitorInsights() {
  ...
  function trackPointer(event: PointerEvent<SVGRectElement>) {
    ...
    setMonthIndex(
      Math.max(
        0,
        Math.min(
          visitors.months.length - 1,
          Math.round(((x - 26) / 288) * (visitors.months.length - 1)),
        ),
      ),
    );
    setHovered(true);
  ...
  return (
    <section
      className={`${styles.card} ${styles.visitors}`}
    ...
    >
      ...
      <svg
        ...
        viewBox="0 0 340 146"
        role="group"
        aria-label="Monthly visitors: loyal, new, and unique customers from January to December"
      >
        ...
        {series.map(({ label, values, color }) => (
          <g key={label}>
            <path
              d={smoothLine(linePoints(values, 288, 98, 400, 26, 12))}
              stroke={color}
              strokeWidth="2.2"
              fill="none"
            />
        ...
        {series.map(({ label, values, color }) => (
          <circle
            key={label}
            cx={marker.x}
            cy={110 - (values[monthIndex] / 400) * 98}
            r={showTooltip ? 3.5 : label === "New Customers" ? 4 : 0}
            fill={color}
            stroke="white"
            strokeWidth={showTooltip ? 1 : 0}
          />
        ))}
        {visitors.months.map((month, i) => (
          <text
            key={month}
            x={26 + (i * 288) / 11}
            y="127"
        ...
        <rect
          ...
          role="slider"
          tabIndex={0}
          aria-label="Visitor insights month"
          aria-valuemin={0}
          aria-valuemax={11}
        ...
        {showTooltip && (
          <g
            transform={`translate(${marker.x > 190 ? marker.x - 133 : marker.x + 9}, 14)`}
            pointerEvents="none"
            aria-hidden="true"
          >
            <rect
              ...
              stroke="#e8e7f2"
            />
            <text x="9" y="13" fill="#171d4d" fontSize="8" fontWeight="600">
              {visitors.months[monthIndex]}
            </text>
