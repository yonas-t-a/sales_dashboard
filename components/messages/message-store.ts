...
const key = "dabang-messages-v1",
  eventName = "dabang-messages-change";
...
function snapshot() {
  try {
    ...
    return (cache = initialConversations);
  }
}
...
export function useMessages() {
  return useSyncExternalStore(subscribe, snapshot, () => initialConversations);
}
function persist(threads: Conversation[]) {
  try {
    ...
    throw new Error(
      "Your browser could not save this change. Check local storage permissions or available space and try again.",
    );
  }
...
export function updateConversation(
  id: string,
  changes: Partial<
    Pick<Conversation, "unread" | "starred" | "archived" | "draft">
  >,
) {
  persist(
    snapshot().map((thread) =>
      thread.id === id ? { ...thread, ...changes } : thread,
    ),
  );
}
export function replyToConversation(id: string, body: string) {
  ...
  if (thread.archived)
    throw new Error("Restore this conversation before replying.");
  persist(
    threads.map((t) =>
      t.id === id
        ? {
...
export function createConversation(
  ...
  if (!contacts.some((c) => c.id === contactId))
    throw new Error("Choose a demo contact.");
