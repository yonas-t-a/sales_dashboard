...
export function ConversationView({
  ...
  function update(changes: Parameters<typeof updateConversation>[1]) {
    try {
      updateConversation(thread.id, changes);
      setError("");
    } catch (error) {
      setError(
  ...
  return (
    ...
    >
      <header className={styles.threadHeader}>
        <button
          className={`${shared.button} ${styles.back}`}
          onClick={onBack}
          aria-label="Back to inbox"
        >
          <ArrowLeft aria-hidden="true" />
        </button>
        <span className={`${shared.avatar} ${shared[contact.color]}`}>
          {contact.name
            .split(" ")
            .map((p) => p[0])
            .join("")}
        ...
        <div className={styles.threadActions}>
          <button
            ...
            onClick={() => update({ starred: !thread.starred })}
          >
            <Star
          ...
          <button
            ...
            onClick={() => update({ unread: !thread.unread })}
          >
            {thread.unread ? (
          ...
          <button
            className={styles.iconButton}
            aria-label={
              thread.archived ? "Restore conversation" : "Archive conversation"
            }
            onClick={() => update({ archived: !thread.archived })}
          >
            {thread.archived ? (
      ...
      <div className={styles.subject}>
        ...
        <span>{thread.archived ? "Archived" : "Conversation"}</span>
      </div>
      <div
      ...
      >
        {thread.messages.map((message) => (
          <article
            key={message.id}
            className={`${styles.message} ${message.author === "you" ? styles.outgoing : ""}`}
          >
            <span>{message.author === "you" ? "You" : contact.name}</span>
            <p>{message.body}</p>
            <time dateTime={message.createdAt}>
              ...
              UTC{message.author === "you" ? " · Saved locally" : ""}
            </time>
          </article>
        ))}
      </div>
      ...
      {thread.archived ? (
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            try {
              replyToConversation(thread.id, draft);
              setDraft("");
              setError("");
              setNotice("Reply saved to this demo conversation.");
            } catch (error) {
              setError(
        ...
        >
          <label className={shared.srOnly} htmlFor="message-reply">
            Write a reply
          </label>
          <textarea
            id="message-reply"
            ...
            onChange={(event) => {
              ...
              setNotice("");
              update({ draft: event.target.value });
            }}
          />
          <div>
            <span>Local demo · no messages are delivered</span>
            <button
              type="submit"
