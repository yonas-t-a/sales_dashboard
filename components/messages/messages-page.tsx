    ...
    >
      <div className={shared.pageHeader}>
        <div>
          <div className={shared.headingLine}>
            ...
            <span className={shared.demoBadge}>Demo inbox</span>
          </div>
          <p>A little closer to your customers and your team.</p>
        </div>
        <button className={styles.primary} onClick={() => setCompose(true)}>
          <Plus aria-hidden="true" />
          New message
        </button>
      </div>
      <div className={styles.inboxOverview}>
        <span>
          <MessageSquare aria-hidden="true" />
          {threads.filter((t) => !t.archived).length} conversations
        </span>
        <span>
          <i />
          {unread} unread
        ...
        <span className={styles.localLabel}>
          Messages and drafts are saved in this browser
        </span>
      </div>
      {error && (
      ...
      <div className={`${styles.workspace} ${thread ? styles.threadOpen : ""}`}>
        <section className={styles.inbox} aria-label="Conversation inbox">
          ...
          >
            {folders.map(({ name, Icon }) => (
              ...
              >
                ...
                {name}
                <span>{filterConversations(threads, name, "").length}</span>
              </button>
            ))}
          </div>
          <div className={styles.threadList}>
            {visible.map((t) => {
              const contact = contactFor(t.contactId),
                last = t.messages[t.messages.length - 1];
              return (
                <button
                  key={t.id}
                  className={`${styles.threadPreview} ${selected === t.id ? styles.selected : ""} ${t.unread ? styles.unread : ""}`}
                  aria-label={`${contact.name}: ${t.subject}${t.unread ? ", unread" : ""}`}
                  aria-current={selected === t.id ? "true" : undefined}
                  onClick={() => {
                    setSelected(t.id);
                    if (t.unread) {
                      try {
                        updateConversation(t.id, { unread: false });
                        setError("");
                      } catch {
                        setError(
                          "This conversation could not be marked as read. Check your browser’s storage permissions.",
                        );
                      }
                    }
                  }}
                >
                  <span
                    className={`${shared.avatar} ${shared[contact.color]}`}
                    aria-hidden="true"
                  >
                    {contact.name
                      .split(" ")
                  ...
                  <span className={styles.previewContent}>
                    ...
                    <span className={styles.previewBody}>
                      {t.draft
                        ? `Draft: ${t.draft}`
                        : `${last.author === "you" ? "You: " : ""}${last.body}`}
                    </span>
                  </span>
                  <span className={styles.previewIndicators}>
                    {t.unread && <i />}
                    {t.starred && (
            ...
            {!visible.length && (
              <div className={styles.emptyList}>
                ...
                <p>
                  {query
                    ? "Try another name, subject, or message."
                    : `There are no conversations in ${folder.toLowerCase()}.`}
                </p>
                {query && (
                  <button
          ...
          <p className={styles.inboxFooter}>
            {visible.length}{" "}
        ...
        {thread ? (
          ...
          >
            ...
            <p>
              Select a conversation to catch up with a customer or your team.
            </p>
            <button className={shared.button} onClick={() => setCompose(true)}>
              <Plus aria-hidden="true" />
              Start a conversation
            </button>
            <small>Demo inbox · messages are not delivered externally</small>
          </section>
        )}
      </div>
      {compose && (
